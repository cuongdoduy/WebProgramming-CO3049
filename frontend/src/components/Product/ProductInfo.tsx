...
const ProductInfo: React.FC<{
  ...
  colors: string[];
  sizes: string[];
}> = ({ id, title, price, image, discount, colors, sizes }) => {
  return (
    <div className="bg-[#F5F5F5] rounded-[4px]">
      ...
      <div className="w-full min-h-[40px] bg-black flex items-center justify-center rounded-b-[4px] cursor-pointer">
        ...
        <Typography as="p" variant="paragraph" className="text-white text-[16px] mx-2">
          Add To Cart
        </Typography>
      </div>
      <div>
        <Typography as="p" variant="paragraph" className="py-1.5 text-[16px] text-[black]">
          {title}
        </Typography>
        <Typography as="p" variant="paragraph" className="text-[16px] text-[#DB4444] font-bold">
